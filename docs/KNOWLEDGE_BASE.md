...
| 7 | **Configuration is read from the worker's disk at startup** | One worker serves one application. `repo`, `trigger` and `integration_model` are declared twice and must agree by hand | Multi-application deployments. Recording *which configuration governed a run* |

...
| 9 | ~~**Only one toolchain image exists**~~ | **Resolved 2026-08-24.** `engine/sandbox/recipes/` holds per-ecosystem recipes and `python -m engine.sandbox build-app --recipe node20 --name X --manifest DIR` builds an image carrying one application's dependencies. Unlike `python312`, a JavaScript image cannot be generic: `next build` runs against *that* application's dependency tree, not against tools. Dependencies install to `/node_modules`, which Node finds by walking up — **not** symlinked into the workspace, because a link created in the container also lands in the host directory `changed_files` reads, and `git add -A` then fails with `unable to index file 'node_modules'` | — |
...
| 12 | **The run record ends at the pull request** | Release opens it and the run finishes. Nothing watches for the merge, so the evidence package cannot say whether code review happened, who merged, or whether the change reached the default branch. [ADR 0009](adr/0009-two-approval-levels.md) makes this the gap that decision creates | Evidence covering both approval levels. Also the first path that needs `control_mode: "observed"`, which invariant 11 defines and nothing yet uses. **Observed concretely 2026-08-24:** run `81aa3d96` was terminated from the Workbench, its branch survived by design, and a human merged it by hand. KuWarden's record says `terminated`; the code is in production, and no `external_effect` row exists. Every commit carries `kuwarden-run-id`, so a reconciliation could find every change KuWarden produced that reached production outside its control |
...
| 15 | ~~**A rejection names no verifier unless the verdict events fired**~~ | **Resolved 2026-08-24, and the cause was not a stale worker.** `_node_step` drained `result.notes` unconditionally, including under `record=False` — a flag that means *the caller emits these*. `_verify`'s `if result.notes` guard was therefore always false, so `verifier_verdict` had **never fired for any run in the system's history**. Two further defects in the same seam: `aborting` recomputed the failing set from `self._latest`, which the parallel fan-out leaves holding whichever branch replied last, so a rejection named an arbitrary verifier — in one observed run, the *advisory* one that could not have caused it; and Compensate, handed the same state, recorded one verifier's findings and destroyed the rest. `verifiers_completed` now carries `falsified_by`, and `_Rejected` carries the blocking set rather than re-deriving it | — |
| 16 | **Trigger health is invisible** | A webhook that stops arriving produces silence, which is indistinguishable from nobody filing a ticket. Observed twice on 2026-08-24: a quick tunnel (`cloudflared tunnel --url`) mints a new hostname on every restart, so the subscription URL goes stale, Azure DevOps accumulates failures and eventually stops trying. Both times the diagnosis was "wait, then ask". The public path can be proven in one request — a payload crafted to be filtered returns `200 {"started": false, "reason": ...}` without starting a run | A `last_delivery_at` on `app_triggers`, written on every received delivery including filtered ones, surfaced on the application page. Turns a silent failure into a visible one |
| 17 | **Verifier findings do not feed back into the Coder** | CI failures reach the next prompt as of 2026-08-24; a verifier rejection still ends the run. A re-run starts from scratch and may reproduce the same mistake, because nothing tells it what was wrong | Recovering from a `correctness` rejection without a human re-reading the findings and rewriting the ticket |
| 18 | **The ticket hears twice, not four times** | Triage acknowledges the run and Reporter posts the outcome. The middle is missing — verified, awaiting approval. Each is an external mutation, so it needs the marker-and-read-back idempotency the acknowledgement already uses; a ticket API offers no idempotency token and a retried activity would otherwise post "picked up" three times | Somebody watching the ticket rather than the Workbench knowing what is happening |
| 19 | **Context selection is one shot, from filenames** | [ADR 0010](adr/0010-context-assembly.md). The model picks paths from a listing before it has read anything, and the pick is frozen for all four inner attempts. It cannot search, and a filename is metadata: ticket 38 said "change the theme to Ocean" and the word `ocean` lives in `app/globals.css` and `lib/site.js`, in neither of their names. Import-graph expansion covers near misses, not indirection | A repository map (paths plus exported symbols, ~1.5% of the cost of contents) closes most of it. Tool-based retrieval closes the rest |
